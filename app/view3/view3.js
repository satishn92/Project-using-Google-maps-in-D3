...
.controller('View3Ctrl', ['settings', '$scope', View3Ctrl]);


function View3Ctrl(settings, $scope) {
  var width = 1250,
    height = 1600,
    svg = null,
    projection = null,
  ...
  function getGeoJson() {
    ...
    d3.json('russia.json', function(error, data) {
      if (error) {
        return console.error(error);
      }
      console.log('russia.json', data);
      ...
      d3.tsv('unemployment.tsv', function(err, dtsv) {
        ...
        legend = svg.append('g')
          .attr('class', 'legend')
          .attr('transform', 'translate(' + (width - 220) + ',' + (100) + ')')
          .selectAll('.color-box')
